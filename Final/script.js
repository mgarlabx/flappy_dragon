...
var abertura = new Image();
abertura.src = 'img/abertura.png';

var gameover = new Image();
gameover.src = 'img/gameover.png';

var inteli = new Image();
inteli.src = 'img/inteli.png';
...
// exibe o fundo
// precisa da função "onload" para esperar carregar a imagem e depois aplicar no Canvas
fundo1.onload = function(){
	context.drawImage(fundo1, fundo.x, fundo.y, fundo.width, fundo.height, fundo.y, fundo.top, fundo.width, fundo.height);
};

// exibe a abertura
abertura.onload = function(){
	context.drawImage(abertura, 100, 50, 200, 150);
};

// exibe o logo do Inteli
inteli.onload = function(){
	context.drawImage(inteli, 350, 10, 40, 40);
};

// exibe a mensagem
document.getElementById('mensagem').innerHTML = 'pressione ENTER para iniciar';

// fica aqui aguardando um ENTER para executar a função Jogar()
...
function Jogar() {
	...
	// não segue adiante se não inciou / reiniciou
	if (jogo.status == 0) return;

	// para o jogo se houve colisão
	if (VerificarColisao()) {
		jogo.status = 0;
		context.drawImage(gameover, 100, 200, 200, 200);
		document.getElementById('mensagem').innerHTML = 'pressione ENTER para reiniciar';
		return;
	}

	// exibe o placar e apaga a mensagem
	...
	// desenha o logo do Inteli (usando sintaxe básica)
	context.drawImage(inteli, 350, 10, 40, 40);
